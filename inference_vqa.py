...
def build_data(args, image_path, question, image_processor, tokenizer, dictionary):
...
def main():
	...
	question = "What is the event in October 9?"

	for i in tqdm(range(len(image_list)), desc='inference'):
		save_path = os.path.join(args.out_dir, os.path.basename(image_list[i]) + '.json')
		src_tokens, src_lengths, img_src_token, img_attn_mask, img_gpt_input_mask, segment_token, p2s_resized_width, p2s_resized_height, raw_width, raw_height = build_data(args, image_list[i], question, image_processor, tokenizer, dictionary)
