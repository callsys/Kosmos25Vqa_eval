...
class LayoutLMCasedPreTrainedModel(PreTrainedModel):
    ...
    config_class = LayoutXLM_line1d_Config
...
class RobertaEncoder(nn.Module):
    ...
    def _cal_1d_pos_emb(self, hidden_states, position_ids, segment_ids, rel_pos_mat):
        if rel_pos_mat == None:
    ...
    def forward(
        ...
        rel_pos = self._cal_1d_pos_emb(hidden_states, position_ids, segment_ids, rel_pos_mat) if self.has_relative_attention_bias else None
...
class LayoutXLM_line1d_ForTokenClassification(LayoutLMCasedPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
...
class LayoutXLM_line1d_ClassificationHead(nn.Module):
    """
...
class LayoutXLM_line1d_ForSequenceClassification(LayoutLMCasedPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        self.layoutlmcased = LayoutLMCasedModel(config, add_pooling_layer=False)
        self.classifier = LayoutXLM_line1d_ClassificationHead(config)
